...
#include <time.h>
#include "hero.h"
#include <conio.h> // getch fonksiyonu kullanýmý için
#include "ates.h"
#include <vector>
#include "dusman.h"
  ...
  if (nesne!=NULL && nesne->getx()<2)
{
    ...
    cout << "kaya: " << nesneyokframe+yeninesneframe;
}
...
///dusman vs malkoc test:
tek=ordu.begin();
while(tek!=ordu.end())
{
    if(malkoc->getx()+3->=(*tek)->getx()&&//sol
    malkoc->getx<=(*tek)->getx()+2&&//sağ
    malkoc->gety()+4>=(*tek)->gety()
    malkoc->gety()+4>=(*tek)->gety()+2)//alt
    {
        devam=false;
    }
}
